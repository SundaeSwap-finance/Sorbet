...
const announceEndpoints = {
  eternl: ["wss://tracker.de-0.eternl.art", "wss://tracker.us-0.eternl.art"],
  peerConnectDemo: [
...
export class SorbetPeerConnect extends CardanoPeerConnect {
  ...
  public static initP2PClient(handler: ConnectionStateChangeHandler, seed?: string) {
    ...
    Log.App.P2PConnect(
      "Created New P2P Client",
      seed ? `with Seed ${seed}` : "without Seed",
      peerConnect
    );

    const logMessage = (func: string, connectMessage: IConnectMessage) =>
      Log.App.P2PConnect(
        `${func} p2p connection state change hook called with message:`,
        connectMessage
      );
