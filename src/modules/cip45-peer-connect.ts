...
import { Cip30DataSignature, IConnectMessage, IWalletInfo, Paginate } from "@fabianbormann/cardano-peer-connect/dist/src/types";
...
class SorbetPeerConnect extends CardanoPeerConnect {
  private w = new ImpersonatedWallet()
...
export function initP2PClient(seed?: string) {
  if (!peerConnect) {
    ...
    peerConnect = new SorbetPeerConnect(seed)
  } else {
    Log.App.P2PConnect("P2P Client already created", peerConnect, connectStatus)
  }
  return peerConnect
}
