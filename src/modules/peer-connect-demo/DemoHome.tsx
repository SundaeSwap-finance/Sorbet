...
const Home = () => {
  ...
  const [meerkatBoostWalletSeed, _unused] = useState(
    localStorage.getItem("meerkat-boostwallet-seed")
  );
  const [boostPeerConnect, setBoostPeerConnect] = React.useState(
    () => new DemoWalletConnect(walletInfo, meerkatBoostWalletSeed, announce)
  );
      ...
      <div style={{ width: "100%" }}>
        ...
        >
          <div style={{ maxWidth: 600, width: "80%" }}>
            <p>
              Paste the dapp identifier into this input field and click the connect button,
              {/* 
                            or{' '}
                            <span
                                style={{
            ...
            {!qrOverlayVisible && (
              <div className="address-container">
                <input
                  ...
                  onChange={(event) => setDAppIdentifier(`${event.target.value}`)}
            ...
            >
              {qrOverlayVisible ? (
                ...
                <Button onClick={connectWithDApp}>Connect</Button>
              ) : (
                <Button onClick={disconnectDApp}>Disconnect</Button>
              )}
            </div>
