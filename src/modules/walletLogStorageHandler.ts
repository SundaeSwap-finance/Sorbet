...
export function getWalletLogScrollback(callback: (scrollbackLength: number) => void): void {
  chrome.storage.local.get([WALLET_LOG_SCROLLBACK_STORAGE_KEY], (result) => {
    const scrollbackLength =
      result[WALLET_LOG_SCROLLBACK_STORAGE_KEY] ?? LOG_SCROLLBACK_LENGTH_DEFAULT;
...
export function retrieveWalletLogs(walletLogCallback: (walletLogs: WalletLog[]) => void): void {
  chrome.storage.local.get(
    [WALLET_LOG_STORAGE_KEY, WALLET_LOG_SCROLLBACK_STORAGE_KEY],
    (result) => {
      const scrollbackLength: number =
        result[WALLET_LOG_SCROLLBACK_STORAGE_KEY] ?? LOG_SCROLLBACK_LENGTH_DEFAULT;
