...
export function useCustomResponse() {
  ...
  useEffect(() => {
    chrome.storage.sync.get(
      ...
      function (result) {
        ...
        _setIsCustomResponseEnabled(
          result[CustomResponseStorageKeys.CUSTOM_RESPONSE_ENABLED] ?? false
        );
      }
    );
  }, []);
  ...
  useEffect(() => {
    return makeStorageChangeListener(
      CustomResponseStorageKeys.CUSTOM_RESPONSE_ENABLED,
      _setIsCustomResponseEnabled
    );
  }, []);
  ...
  return {
    isCustomResponseEnabled,
    setIsCustomResponseEnabled,
    utxos,
    addU,
    setUi,
    removeUi,
